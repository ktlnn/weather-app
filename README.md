...
Weather App uses an OpenWeatherMap API key that allows for the application to render the current weather and a 5 day forecast. The cards are created dynamically and appear on search. The 5 day weather forecast function has a unique for loop that allows loops through the 5 day forecast object provided by the API key and create a card for each day. Each weather card displays the searched city's name, the date, temperature, humidity, wind-speed, and uv index.
